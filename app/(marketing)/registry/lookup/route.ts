...
// to the page the visitor came from with `?miss=1`, which LookupForm renders as the COPY §1.3 miss
// string — never a message that reveals whether some other ID exists. Never cached.
//
...
function missTarget(referer: string | null): string {
  if (!referer) return DEFAULT_MISS_TARGET;
  try {
    return MISS_TARGETS[new URL(referer).pathname] ?? DEFAULT_MISS_TARGET;
  } catch {
    return DEFAULT_MISS_TARGET;
  }
}
...
function answer(request: Request, rawId: string | null): NextResponse {
  ...
  return seeOther(request, missTarget(request.headers.get("referer")));
}
